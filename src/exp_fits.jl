...
function double_exp_loss(u, p)
    x = p[1]
    y = p[2]
    fit = u[1] * (u[2] * exp.(-(1 / u[3]) * x) + (1 - u[2]) * exp.(-(1 / u[4]) * x))
    return sum((fit .- y) .^ 2)
end

optf = Optimization.OptimizationFunction(single_exp_loss, Optimization.AutoForwardDiff())
prob = Optimization.OptimizationProblem(single_exp_loss, [1.0, 1.0], [x, y])
...
prob = Optimization.OptimizationProblem(double_exp_loss, [1.0, 0.9, 1.0, 1.0], [x, y])
OptimizationOptimJL.solve(prob, OptimizationOptimJL.NelderMead(), maxiters=5000, maxtime=100)