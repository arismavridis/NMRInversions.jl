...
function test_lcurve()
    ...
    x = exp10.(range(log10(1e-1), log10(5), 32)) # acquisition range
    X = exp10.(range(-5, 1, 128)) # T range
    ...
    g = K * f_custom
    y = g + 0.01 * maximum(g) .* randn(length(x))

    alphas = exp10.(range(log10(1e-4), log10(1e2), 128))
    ...
    for (i, α) in enumerate(alphas)
        ...
        z = vec(nonneg_lsq(A, [r; zeros(size(A, 1) - size(r, 1))], alg=:pivot))
    ...
    plot(alphas[non_inf_indx], curvatures[non_inf_indx], xscale=:log10)
    ...
    p2 = plot(X, [f, f_custom], label=["solution", "original"], xscale=:log10)
    p3 = plot(rhos, xis, xscale=:log10, yscale=:log10)
